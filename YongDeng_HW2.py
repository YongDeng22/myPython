...
#*****************************************************************************************
	...
	#Author: Yong Deng
	#Since:  4-23-2015
	#This program is a simple search engine which can perform AND and OR searches. 
	#The user can enter multiple words and boolean operators (and, or).
	#If user only entered the "or" operator, an OR search will be performed.
	#Otherwise, an AND search will be performed.
	#An empty query or a one that only contains boolean operators is invalid.
	#The user will be prompted to enter query again.
#*****************************************************************************************
query=input("query:")	#prompt the user to enter query and convert to lower case
...
#if the user does not enter anything, prompt the user to enter again
while (len(query) == 0):
	query = input("Query cannot be empty. Please enter a query: ").lower()

#print search info
print("\nPerforming", operation, "search for:", query, "\n")
found = 0	#track whether any match found. If yes > 0, otherwise equals to 0
	...
	for q in query:
		if (operation == "OR"):
			...
			if (q not in words):
				...
				break
	if (andFound == "yes"):
		#print result of "AND" search
		print("Found at", i, quote[:80])
		found = found + 1	#when find a match, increment found

#when program finish, print ststistical info
if (found == 0):
	#when no match found, print message
	print ("No matches found.\n")
else:
	#if matches found, print the number of matches
	print("")
